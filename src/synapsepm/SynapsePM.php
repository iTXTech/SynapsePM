	...
	public function onEnable() {
		...
		if ($useOldGenisysConfig) {
			...
			$this->getConfig()->set('synapses', [
				[
					'enabled' => $this->getServer()->getAdvancedProperty('synapse.enabled', false), 'server-ip' => $this->getServer()->getAdvancedProperty('synapse.server-ip', '127.0.0.1'), 'server-port' => $this->getServer()->getAdvancedProperty('synapse.server-port', 10305), 'is-main-server' => $this->getServer()->getAdvancedProperty('synapse.is-main-server', true), 'server-password' => $this->getServer()->getAdvancedProperty('synapse.server-password', '123456'), 'description' => $this->getServer()->getAdvancedProperty('synapse.description', 'A Synapse client')
				]
			]);
